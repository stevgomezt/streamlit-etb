                        ...
                        for config in configuraciones:
                            ...
                            if mayus==True:
                                bar = alt.Chart(df_group).mark_bar().encode(
                                    ...
                                    y=alt.Y(config['y_axis'] + ":N", sort=list(df_group[config['y_axis']]),axis=alt.Axis(ticks=False)),#, title=None
    ...
    if b == True:
        ...
        with vista1:    # Modelo Multiples Clientes
            try:                  
                ...
                col111,col112,col113,col114 = st.columns(spec=[1,1,1,1]) #col112, 
                ...
                df_tmp['Top 3'] = hm_df[[ 'Producto_1', 'Producto_2', 'Producto_3']].sum(axis=1)
...
############# ------------------------------------------------ 
                    ######## INSTALACIONES
                dona_plotly(producto = 'INSTALACIONES',titulo='Instalaciones' ,col = col111)
                
                # ######## Mantenimiento
                # dona('MANTENIMIENTO',0 , 1, 'Mantenimiento')
                dona_plotly(producto = 'MANTENIMIENTO', titulo='Mantenimiento',col = col112)
                ...
                dona_plotly(producto = 'AUMENTOS_CARGA',titulo='Aumentos de carga', col = col114)

                # #FIBRA OPTICA
                # dona('FIBRA_OPTICA',1 , 0, 'Fibras ópticas')
                dona_plotly(producto = 'FIBRA_OPTICA', titulo='Fibras ópticas', col = col111)

                # #REDESELECTRICAS
                # dona('REDESELECTRICAS',1 , 1, 'Redes eléctricas')
                dona_plotly(producto = 'REDESELECTRICAS',titulo = 'Redes eléctricas' ,col = col112)

                # #ILUMINACION
                # dona('ILUMINACION',1 , 2, 'Iluminación')
                dona_plotly(producto = 'ILUMINACION',titulo = 'Iluminación' , col = col113)

                # #CUENTASNUEVAS
                # dona('CUENTASNUEVAS',1 , 3, 'Cuentas nuevas')
                dona_plotly(producto = 'CUENTASNUEVAS',titulo = 'Cuentas nuevas' , col = col114)
